# Nibandha Config & Initialization Rule
...
- **Benefit:** This ensures the logic remains identical whether the library is running in "Standalone Mode" (loading its own YAML) or "Client Mode" (receiving a Pydantic object from Amsha).
...
- **Failure Mode:** If a required setting is missing and no default exists, raise a `NibandhaConfigError`. 
