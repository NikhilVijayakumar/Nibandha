# Nibandha Logging & Anti-Print Rule

## 1. Prohibited Output
- **No `print()`:** The use of the `print()` function is strictly forbidden in `src/`.
- **Reason:** As a library, Nibandha must remain silent unless the client (Amsha/Pravaha) explicitly checks the logs.
...
- **Levels:** - `INFO`: For successful lifecycle events (e.g., "Module X bound").
    - `DEBUG`: For granular logic steps (e.g., "Calculated next ID as CP-UT-005").
    - `ERROR`: For failures, including the stack trace where applicable.

## 3. Reporting Integration
- **Rule:** Logging should be structured to support the **Reporting Module**.
- **Requirement:** When generating a report, the agent must be able to parse logs to identify "Top Offenders" or "Failure Patterns."