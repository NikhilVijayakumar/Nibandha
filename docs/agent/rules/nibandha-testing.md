...
- `{id}`: Lowercase Scenario ID from docs (e.g., `UR-UT-001` becomes `ur_ut_001`).
- `{slug}`: A brief snake_case description.
- **Correct Example:** `test_ur_ut_001_extraction.py`
- **Correct Example:** `test_rotation_005_limit_reached.py`

## 3. Workflow: Documentation-First
Before creating or modifying a test:
1.  **Read/Update Documentation:** Check `docs/test/<module>/unit_test_scenarios.md` (or `e2e`).
