...
| **Type Hint** | A Python annotation indicating the expected type of a variable or function |
| **Type Annotation** | The syntax used to add type hints to Python code (e.g., `def func(x: int) -> str:`) |
| **mypy** | A static type checker for Python that validates type hints |
| **Strict Mode** | mypy configuration that enables all optional checks for maximum type safety |
| **Type Error** | A mismatch between expected and actual types detected by mypy |
| **Any Type** | A special type that disables type checking (should be avoided in strict mode) |
